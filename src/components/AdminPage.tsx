...
export default function AdminPage() {
  ...
  if (!isLogged) {
    // شاشة تسجيل الدخول باللون الأحمر الداكن
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-red-900 p-6 text-center">
        ...
        />
        <button onClick={checkPassword} className="bg-red-600 text-white px-8 py-3 rounded-xl font-bold hover:bg-red-700">
          دخول
        </button>
      </div>
    );
  ...
  return (
    <div className="fixed inset-0 bg-[#b91c1c] z-[100] flex flex-col items-center justify-center p-8">
      ...
      <button className="bg-white text-[#b91c1c] px-10 py-4 rounded-2xl font-bold text-xl hover:bg-gray-100 transition-all">
        تحميل تطبيق الإدارة
      </button>
    </div>
  );
